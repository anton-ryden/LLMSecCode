...
def parse_args():
    ...
    parser.add_argument('--template_set', choices=['llama', 'codewizard'], default='llama',
...
def get_templates(template_set):
    # Needs to fixed in future load from file for example
    ...
    #"{% for message in messages %}{% if message['role'] == 'user' %}{{ '### Instruction: ' + message['content'] + '\\n'}}{% elif message['role'] == 'system' %}{{ message['content'] + '\\n\\n' }}{% elif message['role'] == 'assistant' %}{{ '### Response: '  + message['content'] + ' '}}{% endif %}{% endfor %}"
